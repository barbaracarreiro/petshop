package com.petshop.petshop;


public class PetshopController {

    import java.util.List;
    import java.util.Optional;
    
    import org.springframework.beans.factory.annotation.Autowired;
    import org.springframework.http.HttpStatus;

    import org.springframework.http.ResponseEntity;
    import org.springframework.web.bind.annotation.DeleteMapping;

    import org.springframework.web.bind.annotation.GetMapping;
    import org.springframework.web.bind.annotation.PathVariable;

    import org.springframework.web.bind.annotation.PostMapping;
    import org.springframework.web.bind.annotation.PutMapping;

    import org.springframework.web.bind.annotation.RequestBody;
    import org.springframework.web.bind.annotation.RequestMapping;
    
    import org.springframework.web.bind.annotation.RestController;
    import br.com.tech4me.petshop.model.Petshop;
    import br.com.tech4me.petshop.service.PetshopService;


    @RestController
    @RequestMapping("/sistema")
    public class PetshopController {
        @Autowired
        private PetshopService servico;
    
        @PostMapping
        public ResponseEntity<Petshop> cadastrarPet (@RequestBody Petshop petshop){
            return new ResponseEntity<>(servico.cadastrarPet(petshop),HttpStatus.CREATED);
        }
        @GetMapping("/{id}")
        public ResponseEntity<Petshop> ObterPet(@PathVariable String id){
            Optional<Petshop> retorno = servico.ObterPorId(id);
            if (retorno.isPresent()){
                return new ResponseEntity<>(retorno.get(),HttpStatus.FOUND);
            }else{
                return new ResponseEntity<>(HttpStatus.NOT_FOUND);
            }
        }
            @GetMapping
            public ResponseEntity <List<Petshop>> ObterPet(){
                return new ResponseEntity<>(servico.ObterPets(),HttpStatus.OK);
            }
    
            @DeleteMapping("/{id}")
            public ResponseEntity<Void> excluirPet(@PathVariable String id){
                servico.excluirPorId(id);
                return new ResponseEntity<>(HttpStatus.NO_CONTENT);
            }
            @PutMapping("/{id}")
            public ResponseEntity<Petshop> atualizarPorId(@PathVariable String id,@RequestBody Petshop petshop){
                Optional<Petshop> retorno= servico.atualizarPorId(id, petshop);
                if(retorno.isPresent()){
                    return new ResponseEntity<>(retorno.get(),HttpStatus.ACCEPTED);
                }else{
                    return new ResponseEntity<>(HttpStatus.NOT_ACCEPTABLE);
                    }   
            }  
        }     
}
